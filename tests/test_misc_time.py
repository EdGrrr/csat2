...
class TestLSTConversion(unittest.TestCase):
    def test_dataIsGMT(self):
        ...
        self.assertTrue((time.utc_to_lst(output, longitudes) == lst_time).all(),
    ...
    def test_raiseErrors(self):
        ...
        self.assertRaises(
            ValueError,
            time.toLocalSolarTime, lst_time, gmt_times, longitudes, data)
...
class TestTimeConversion(unittest.TestCase):
    def test_tai_utc_conversion(self):
        ...
        self.assertEqual(time.tai_to_utc(
            time.utc_to_tai(2001, 1, 0)), (2001, 1, 0))
        self.assertEqual(time.utc_to_tai(*time.tai_to_utc(20000000)), 20000000)

    def test_utc_sat_offset(self):
        self.assertEqual(time.utc_to_sat_offset(
            np.array([13.5]), np.array([0]), 13.5), 0)
        self.assertEqual(time.utc_to_sat_offset(
            np.array([13.5]), np.array([-15]), 13.5), -1)
        assert time.utc_to_sat_offset(np.array([13.5]), np.array([15]), 13.5) == 1
        assert time.utc_to_sat_offset(np.array([13.5]), np.array([345]), 13.5, col='6') == -1

        assert time.utc_to_sat_offset(np.array([13.5]), np.array([195]), 13.5, col='5') == 13
        assert time.utc_to_sat_offset(np.array([13.5]), np.array([195]), 13.5, col='6') == -11
