...
class TestECMWFDownload(unittest.TestCase):
    def test_ECMWFDownload(self):
        ...
        assert ECMWF.check(
            2020, 1, ["Temperature"], level="700hPa", resolution="1grid", days=[1, 2]
        )[0]
    ...
    def test_ECMWFDownloadLST(self):
        ...
        assert ECMWF.check(
            2020, 1, ["Temperature"], level="1000hPa", resolution="1grid", days=[1, 2]
        )[0]
        assert ECMWF.check(
            2020,
            1,
...
class TestECMWFBasic(unittest.TestCase):
    ...
    def test_eis_calculations(self):
        ...
        # LTS
        assert self.rel_err(ECMWF.ECMWF._calc_lts(t700, t1000), 9.78815) < 0.01
        # MALR
        assert (
            self.rel_err(
        ...
        # LCL
        assert self.rel_err(ECMWF.ECMWF._calc_lcl(t1000, rh1000), 433.884) < 0.01
        # EIS - This is not Rob's number as I am not sure quite how he gets it
        assert self.rel_err(ECMWF.ECMWF._calc_eis(t700, t1000, rh1000)[1], -1.8) < 0.01

    def test_ERA5_derived(self):
        # Check doesn't throw error
        data = ECMWF.readin(
            "ERA5", 2020, 1, "LTS", level="1000hPa", time="timed", resolution="1grid"
        )
        assert data.shape == (8, 360, 180)

        data = ECMWF.readin(
            "ERA5", 2020, 1, "EIS", level="1000hPa", time="timed", resolution="1grid"
        )
        assert data.shape == (8, 360, 180)
...
class TestECMWFData(unittest.TestCase):
    ...
    def test_era5data_timeslice(self):
        ...
        temp = tdata.get_data_time(misc.time.ydh_to_datetime(2020, 1, 6))
        ...
        temp2 = tdata.get_data_time(misc.time.ydh_to_datetime(2020, 2, 6))
    ...
    def test_era5data_timeslice_timeinterp(self):
        tdata = ECMWF.ERA5Data(
            "Temperature", "1000hPa", res="1grid", linear_interp=False
        )
        temp_pre = tdata.get_data_time(misc.time.ydh_to_datetime(2020, 1, 0))
        temp_post = tdata.get_data_time(tdata.time[1])
        # Ensure these are not the same data!
        assert np.any(temp_pre != temp_post)

        tdatai = ECMWF.ERA5Data(
            "Temperature", "1000hPa", res="1grid", linear_interp="time"
        )
    ...
    def test_era5data_points(self):
        ...
        temp = tdata.get_data(
            [140, -40], [-25, -25], misc.time.ydh_to_datetime(2020, 1, 6)
        )
        assert temp[0] > temp[1]

        # North pole is colder than south pole
        temp = tdata.get_data([0, 0], [89, -89], misc.time.ydh_to_datetime(2020, 1, 6))
        assert temp[0] < temp[1]
    ...
    def test_era5data_points_simple(self):
        ...
        # Australia is warmer than the Pacific
        temp = tdata.get_data(
            [140, -40], [-25, -25], misc.time.ydh_to_datetime(2020, 1, 6), simple=True
        )
        assert temp[0] > temp[1]

        # North pole is colder than south pole
        temp = tdata.get_data(
            [0, 0], [89, -89], misc.time.ydh_to_datetime(2020, 1, 6), simple=True
        )
        assert temp[0] < temp[1]
    ...
    def test_era5data_points_timeinterp(self):
        ...
        _ = tdata.get_data(locs_lon, locs_lat, misc.time.ydh_to_datetime(2020, 1, 0))
        temp_pre = tdata.get_data(locs_lon, locs_lat, tdata.time[0])
        temp_post = tdata.get_data(locs_lon, locs_lat, tdata.time[1])
        assert np.any(temp_pre != temp_post)
        ...
        for tw in [0, 0.1, 0.5, 0.9, 0.99]:
            temp = tdatai.get_data(
                locs_lon, locs_lat, tdata.time[0] + tw * (tdata.time[1] - tdata.time[0])
            )
            assert np.max(np.abs(tw * (temp_post - temp_pre) + temp_pre - temp)) < 0.01
    ...
    def test_era5data_points_spaceinterp(self):
        ...
        temp_ll = tdata.get_data(rdown(locs_lon), rdown(locs_lat).clip(-90, 90), ttime)
        temp_lr = tdata.get_data(
            rdown(locs_lon + 1), rdown(locs_lat).clip(-90, 90), ttime
        )
        temp_ul = tdata.get_data(
            rdown(locs_lon), rdown(locs_lat + 1).clip(-90, 90), ttime
        )
        temp_ur = tdata.get_data(
            rdown(locs_lon + 1), rdown(locs_lat + 1).clip(-90, 90), ttime
        )
        ...
        ) + (1 - weights_lat) * (weights_lon * temp_ul + (1 - weights_lon) * temp_ur)

        tdatai = ECMWF.ERA5Data(
            "Temperature", "1000hPa", res="1grid", linear_interp="space"
        )
