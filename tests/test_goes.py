...
class TestGoesGranule(unittest.TestCase):
    def setUp(self):
        ...
        for ch in [1, 13]:
            ...
            if len(newfiles) != 1:
                pytest.skip(
                    "Test requires G16.2018002.0000.RadC (ch{ch})- run with --runnetwork to download automatically".format(
                        ch=ch
                    )
                )
    ...
    def test_readin_filename(self):
        data = GOES.readin_radiances_filename(self.gran.get_filename(channel=13))
