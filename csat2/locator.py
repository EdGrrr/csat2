...
class FileLocator:
    ...
    * can be used as a wildcard (e.g. for MODIS files that include the processing date
        ...
        for line in searchfiledata:
            ...
            if line == "" or line[0] == "#":
                ...
                for product in products:
                    ...
                    (
                        search_paths[origin][product].append(
                            partial_format(line.strip(), macro)
                        )
                    )
    ...
    def get_folder(self, origin, product, return_primary=True, *args, **kwargs):
        ...
        patterns = self.get_patterns(origin, product, **kwargs)
        patterns = [self.pattern_format(os.path.dirname(p), **kwargs) for p in patterns]
        if return_primary:
            return patterns[0]
        else:
            return patterns
    ...
    def format_filename(self, origin, product, return_primary=True, *args, **kwargs):
        ...
        patterns = self.get_patterns(origin, product, **kwargs)
        patterns = [
            self.pattern_format(p, **kwargs).replace("*", "0") for p in patterns
        ]
        if return_primary:
            return patterns[0]
        else:
    ...
    for machine in config["machines"].keys():
        if fnmatch.fnmatch(_hostname, machine):
            ...
            if os.path.exists(mfile):
                ...
                mdata = pkgutil.get_data(__name__, "config/{}".format(spfile)).decode(
                    "ascii"
                )
                mdata = mdata.split("\n")
                print(mdata)
