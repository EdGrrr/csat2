...
def ydh_to_datetime(year, doy, hour):
    ...
    dat = datetime.datetime(*doy_to_date(year, doy)) + datetime.timedelta(
        hours=int(hour // 1), minutes=int(minutes), seconds=int(seconds)
    )
    return dat
...
def tai_to_utc(time_tai):
    ...
    time_utc = datetime.datetime(1993, 1, 1) + datetime.timedelta(seconds=time_tai)
    return (
        time_utc.year,
        time_utc.timetuple().tm_yday,
        time_utc.hour + time_utc.minute / 60.0 + time_utc.second / 3600.0,
    )
...
def utc_to_tai(year, doy, time_utc):
    ...
    td = (
        ...
        + datetime.timedelta(hours=time_utc)
...
def lst_to_utc(time_lst, longitude):
    ...
    return np.mod(time_lst - longitude * 24.0 / 360, 24)


def utc_to_lst(time_utc, longitude):
    """Returns the LST time given a decimal UTC and longitude"""
    longitude = np.mod(longitude, 360.0)
    return np.mod(time_utc + longitude * 24.0 / 360, 24)
...
def toLocalSolarTime(
    lst_time, gmt_times, longitudes, data, interpolation="linear", DEBUG=False
):
    ...
    if len(set(gmt_times)) != len(gmt_times):
        raise ValueError(
            "Data should not contain duplicate times" + "(gmt_times,toLocalSolarTime))"
        )
    ...
    for lon_ind in range(len(longitudes)):
        ...
        if local_gmt > times_sort[-1]:
            raise ValueError("Cannot calculate LST, require extra data timeslice")
        ...
        if high_ind == low_ind:
            ...
            if interpolation == "linear":
                ...
                if abs(longitudes[lon_ind] - longitudes[low_ind]) < abs(
                    longitudes[lon_ind] - longitudes[low_ind]
                ):
                    weights[low_ind] = 1
                else:
                    weights[high_ind] = 1

        outdata[:, lon_ind] = (data[:, lon_ind, :] * weights[np.newaxis, :]).sum(axis=1)
