...
def files_available(product, year, doy, rev):
    filenames = locator.search(
        "CLOUDSAT", product, year=year, doy=doy, rev=rev, orbit="*****"
    )
...
def get_orbit_date_approx(orbit):
    ...
    test_orb = index_date + timedelta(days=((orbit - index_orbit) / orbits_per_day))
...
def readin_cloudsat_curtain(
    product, year=None, doy=None, orbit=None, sds=None, col=DEFAULT_COLLECTION
):
    ...
    filename = locator.search(
        "CLOUDSAT", product, year=year, doy=doy, mon=mon, day=day, col=col, orbit=orbit
    )[0]
