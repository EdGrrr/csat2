...
from .readfiles import readin_cloudsat_curtain, get_orbit_by_time, get_orbit_date, get_orbit_datetime, DEFAULT_COLLECTION
...
class Granule(object):
    ...
    def get_variable(self, product, varnames, col=None):
        if not col:
            col = self.col
        return readin_cloudsat_curtain(
            product, sds=varnames, col=col, orbit=self.orbit)
    ...
    def download_product(self, product, col=None):
        ...
        if not check(product, year, doy,
                     orbit=self.orbit, col=col):
