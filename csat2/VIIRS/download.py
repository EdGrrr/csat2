...
def download_file_locations_nrt(product, year, doy, times=None, col=DEFAULT_COLLECTION):
    ...
    laads_folder = (
        base_url + f"/api/v2/content/details/allData/{col}/{product}/{year}/{doy:0>3}/"
    )

    token = get_token()
    files = [
        base_url + a["downloadsLink"]
        for a in json.loads(
            geturl(laads_folder + "?fields=all&formats=json", token).decode("utf-8")
        )["content"]
    ]
    return files
...
def download(product, year, doy, times=None, col="5110"):
    ...
    local_folder = locator.get_folder(
        "VIIRS", product, year=year, doy=doy, collection=col
    )
...
def download_geometa(year, doy, sat, col=DEFAULT_COLLECTION, force_redownload=False):
    ...
    laads_file = (
        ...
        + "{col}/{sat}/{year}/{si}03MOD_{year}-{mon:0>2}-{day:0>2}.txt".format(
            ...
            mon=mon,
            day=day,
            si={"NPP": "VNP", "JPSS1": "VJ1"}[sat],
        )
    )

    local_file = locator.format_filename(
        "VIIRS", "GEOMETA", year=year, doy=doy, sat=sat
    )
