...
def _create_bowtie_correct(img_data, mod_data, output_file, res=375):
    ...
    inc_lat = (mod_data['latitude'][centre_scan_row+scan_width] -
               mod_data['latitude'][centre_scan_row])/scan_width

    lon = (
        np.arange(-np.floor(scan_width//2),
                  -np.floor(scan_width//2)+scan_width)[:, None] *
        inc_lon[None, :]) + mod_data['longitude'][centre_scan_row][None, :]
    lat = (
        np.arange(-np.floor(scan_width//2),
                  -np.floor(scan_width//2)+scan_width)[:, None] *
    ...
    for j in range(scan_width):
        along_track_index[j, :] = np.argmin(
            csat2.misc.geo.haversine(lat[j, :], lon[j, :],
                                     old_lat, old_lon), axis=0)
