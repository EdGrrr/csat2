...
def readin_VIIRS_L2(product, year, doy, sds=None, time=None,
                    col=None, varind=None, scale=True):
...
def readin_metadata(product, year, doy, time, col, sds):
    if isinstance(time, int):
        time = '{:0>4}'.format(time) # time should be a string to cope with leading zeros
    filename = locator.search('VIIRS', product, year=year, doy=doy,
                              collection=col, time=time)[0]
    with Dataset(filename) as ncdf:
        group = netcdf_search_groups(ncdf, sds)
        return {a: ncdf.groups[group].variables[sds].getncattr(a) for a in ncdf.groups[group].variables[sds].ncattrs()}
...
def readin_VIIRS_GEOMETA(year, doy, sat='N'):
    ...
    filenames = locator.search('VIIRS', 'GEOMETA', year=year, mon=mon,
                               day=day, sat=satname)
    if len(filenames) == 0:
...
def list_available_times(product, year, doy, col=None):
    ...
    filenames = sorted(locator.search(
        'VIIRS', product, year=year, doy=doy, collection=col, time='*'))
