...
def create_advection_files(sat='G16', area='RadC', gran=None):
    ...
    for band, res in [bands[-1]]:
        ...
        newpos = gran.locate(np.array(list(zip((lon+dlon).ravel(), lat.ravel()))),
                             interp=True)
        ...
        newpos = gran.locate(np.array(list(zip((lon).ravel(), (lat+dlat).ravel()))), interp=True)    
        newpos_dxlat = (newpos[:, 0].reshape(lon.shape) - oldpos[0])
        newpos_dylat = (newpos[:, 1].reshape(lon.shape) - oldpos[1])

        filename = (locator.get_folder('GOES', 'advection', sat=sat, area=area, res=res) +
        ...
        with Dataset(filename, 'w', format='NETCDF4') as ncdf:
            ...
            for varname, outdata in [['newpos_dxlat', newpos_dxlat],
...
class GOESAdvector():
    def __init__(self, gran=None, sat='G16', area='RadC', interp=True):
        ...
        filename = locator.search('GOES', 'advection', sat=self.gran.sat, area=self.gran.area, res=2)[0]
        with Dataset(filename) as ncdf:
            ...
            self.data = {}
            for name in ['newpos_dxlat', 'newpos_dxlon',
                         'newpos_dylat', 'newpos_dylon']:
    ...
    def advect_field(self, data, udist, vdist, band):
        ...
        xinc = (udist/10)*self.data['newpos_dxlon']+(vdist/10)*self.data['newpos_dxlat']
        yinc = (udist/10)*self.data['newpos_dylon']+(vdist/10)*self.data['newpos_dylat']
        ...
        nx_ind = x_ind+xinc
        ny_ind = y_ind+yinc
        clipmask = (nx_ind<0)*(ny_ind<0)*(nx_ind>data.shape[1])*(ny_ind>data.shape[0])
        output = data[np.clip(nx_ind, 0, data.shape[1]),
                      np.clip(ny_ind, 0, data.shape[0])]
        output[clipmask] = np.nan
        return output

    def advect_pixels(self, pixels, udist, vdist):
        xinc = ((udist/10)*self.data['newpos_dxlon'][pixels[0], pixels[1]] +
                (vdist/10)*self.data['newpos_dxlat'][pixels[0], pixels[1]])
        yinc = ((udist/10)*self.data['newpos_dylon'][pixels[0], pixels[1]] +
                (vdist/10)*self.data['newpos_dylat'][pixels[0], pixels[1]])
