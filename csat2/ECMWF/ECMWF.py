    ...
    **kwargs,
    ...
    if resolution == "1grid":  # and (returndata['time'].dtype == np.float):
        ...
        with Dataset(filename[0]) as ncdf:
            ...
            try:
                rtime = num2date(
                    ncdf.variables["time"][:], units=ncdf.variables["time"].units
                )
            except AttributeError:  # Times in files is in hour strings
                rtime = np.array(
                    [
                        ydh_to_datetime(year, doy, (t // 100) + (t % 100) / 60)
            ...
            return xr.DataArray(
                np.swapaxes(returndata, 1, 2)[:, cycle][:, :, ::-1],
                name=variable_names(ncdf),
                coords=[
                    ("time", rtime),
                    ("lon", rlon),
...
def _td(temp, rh):
    ...
    return (
        C
        * (np.log(rh / 100) + (B * temp) / (C + temp))
        / (B - np.log(rh / 100) - B * temp / (C + temp))
        + 273.15
    )
...
def _sat_adiabatic_potential_gradient(T, p, rh):
    ...
    ep = 0.622
    es = (
        6.112 * np.exp((17.67 * (T - 273.15)) / ((T - 273.15) + 243.5)) * 100
    )  # From Bolton
    qs = ep * es / (p - es)
...
def available(variable=None, year=None, res=None, time=None):
    ...
    if variable is None:
        ...
        variables = list(
            set(["_".join(f.split("/")[-1].split("_")[:-2]) for f in files])
        )
        variables.sort()
        ...
        for level in levels_unique:
            doy_counts = np.histogram(
                (doys[levels == level]), bins=np.arange(1, 367, 5)
            )[0]
...
class ERA5Data:
    ...
    def get_data_time(self, time):
        ...
        if self.linear_interp in ["time", "both"]:
            ...
            if time_ind <= len(self.time) - 1:
                ...
                weight = 1 - (
                    (time - self.time[time_ind])
                    / (self.time[time_ind + 1] - self.time[time_ind])
                )
                u_ret = (weight * self.nc.variables[self.varkey][time_ind, :, :]) + (
                    (1 - weight) * self.nc.variables[self.varkey][time_ind + 1, :, :]
                )
            else:
        ...
        if self.res == "1grid":  # Arrange the data to match the MODIS files
            ...
            rlon = np.where(rlon > 180, rlon - 360, rlon)
            return xr.DataArray(
                np.swapaxes(u_ret, 0, 1)[cycle][:, ::-1],
                coords=[("lon", rlon), ("lat", self.lat[::-1])],
            )
        else:
            return xr.DataArray(
                u_ret, dims=["lat", "lon"], coords={"lon": self.lon, "lat": self.lat}
            )
    ...
    def get_data(self, lon, lat, time, simple=False):
        ...
        if self.linear_interp in ["space", "both"]:
            ...
            lat_weight = ((self.lat[lat_ind] - 1) / self.lat_inc)[..., None]
        ...
        if simple:
            if len(self.nc.variables[self.varkey].shape) > 3:
                u_ret = (
                    self.scaling
                    * self.nc.variables[self.varkey][time_ind, 0, :, :][
                        lat_ind, lon_ind
                    ]
                )
            else:
                u_ret = (
                    self.scaling
                    * self.nc.variables[self.varkey][time_ind, :, :][lat_ind, lon_ind]
                )
            return u_ret
            ...
            if len(self.nc.variables[self.varkey].shape) > 3:
                u_ret = (
                    self.scaling
                    * self.nc.variables[self.varkey][
                        time_ind,
                        0,
                        lat_range[0] : lat_range[1],
                        lon_range[0] : lon_range[1],
                    ][lat_ind - lat_range[0], lon_ind - lon_range[0]]
                )
                if t_interp:
                    u_retp = (
                        self.scaling
    ...
    def update_files(self, year, doy):
        ...
        self.time = num2date(
            self.nc.variables["time"][:], units=self.nc.variables["time"].units
        )
...
class ERA5Data3D:
    ...
    def get_data_time(self, time, level):
        ...
        return (
            self.data[ind_upper].get_data_time(time) * (1 - frac)
    ...
    def get_data(self, lon, lat, level, time, simple=False):
        ...
        for ind in range(len(self.levels)):
            mask = ind_lower == ind
            if mask.sum() > 0:
                output[mask] += (1 - frac[mask]) * self.data[ind].get_data(
                    lon[mask], lat[mask], time, simple=simple
                )
            mask = ind_upper == ind
            if mask.sum() > 0:
                output[mask] += (frac[mask]) * self.data[ind].get_data(
                    lon[mask], lat[mask], time, simple=simple
                )
        return output
...
class ERA5WindData:
    def __init__(
        ...
        self.udata = ERA5Data(
            varname_u, level=level, res=res, scaling=wind_scaling[0], *args, **kwargs
        )
        self.vdata = ERA5Data(
            varname_v, level=level, res=res, scaling=wind_scaling[1], *args, **kwargs
        )

    def get_data_time(self, time):
        """Returns the wind fields at a given time"""
        return (self.udata.get_data_time(time), self.vdata.get_data_time(time))

    def get_data(self, lon, lat, time, simple=False):
        return (
            self.udata.get_data(lon, lat, time, simple=simple),
            self.vdata.get_data(lon, lat, time, simple=simple),
        )
...
class ERA5WindData3D:
    ...
    def get_data_time(self, time, level):
        """Returns the wind fields at a given time"""
        return (
            self.udata.get_data_time(time, level),
            self.vdata.get_data_time(time, level),
        )

    def get_data(self, lon, lat, level, time, simple=False):
        return (
            self.udata.get_data(lon, lat, level, time, simple=simple),
            self.vdata.get_data(lon, lat, level, time, simple=simple),
        )
...
def readin_MACCanth(year, doy):
    filename = locator.search("ECMWF", "MACC-anth", year=year, doy=doy)
    try:
        fname = filename[0]
        with Dataset(fname, "r") as ncdf:
            ...
            cycle = list(range(180, 360)) + list(range(180))
            outdata["aod"] = (
                ncdf.variables["aod"][:].squeeze().transpose()[cycle][:, ::-1]
            )
            outdata["aaod"] = (
                ncdf.variables["anthaod"][:].squeeze().transpose()[cycle][:, ::-1]
            )
            return outdata
    except IndexError:
        return None
...
def readin_MACC_aod(year, doy, sds, time="LST"):
    ...
    try:
        ...
        with Dataset(fname, "r") as ncdf:
            ...
            for name in sds:
                outdata[name] = (
                    ncdf.variables[name][:].squeeze().transpose()[cycle][:, ::-1]
                )
            return outdata
    except IndexError:
        return None
