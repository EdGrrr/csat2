...
def make_composite(
    gran, filename, bands=None, scale=None, offset=None, gamma=None, imscale=1, dpi=200
):
    ...
    toplot = np.concatenate(
        (refl_blu[:, :, None], btd[:, :, None], bt[:, :, None]), axis=2
    )
...
def bowtie_correct(field_1km):
    ...
    with Dataset(file_correct) as ncdf:
        ...
        field_1km.values = field_1km.values[
            along_track_index.astype("int"), cross_track_index.astype("int")
        ]
        return field_1km
...
def _create_bowtie_correct(mod_data, output_file):
    ...
    for i in range(lon.shape[0] // 10):
        ...
        lat[(10 * i) : (10 * (i + 1))] = (
            centre_lat + np.arange(-9, 10, 2)[:, None] * inc_lat[None, :]
        )
    ...
    for j in range(distance_from_baseline.shape[1]):
        sdata = list(
            zip(distance_from_baseline[:, j], range(distance_from_baseline.shape[0]))
        )
        sdata.sort()
...
def _create_crosstrack_rectify(mod_data, output_file):
    ...
    remapped_ratio = np.zeros((int(dist.max() + 1), 2)) + np.array((len(dist) - 2, 1))
...
def _point_side_line(pt, ln0, ln1):
    d = (pt[0] - ln0[0]) * (ln1[1] - ln0[1]) - (pt[1] - ln0[1]) * (ln1[0] - ln0[0])
    return d
...
def _point_intersection_box(pts, boxpoints):
    ...
    npts = np.concatenate((npts, npts[0][None, :]))
...
def point_intersection_granule(pts, granpoints):
    ...
    for i in range(1, 4):
        newpoints.append(
            csat2.misc.geo.coordinate_rotate(
                granpoints[i][0] + inc, granpoints[i][1], *initial_loc
            )
        )

    pts_rotate = csat2.misc.geo.coordinate_rotate(pts[0] + inc, pts[1], *initial_loc)
