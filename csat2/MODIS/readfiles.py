...
def readin(product, *args, **kwargs):
    ...
    elif product in ['MODATML2', 'MYDATML2',
                     'MOD021KM', 'MYD021KM',
                     'MOD02HKM', 'MYD02HKM',
                     'MOD02QKM', 'MYD02QKM',
                     'MOD03', 'MYD03',
...
def readin_MODIS_L3_filename_xarray(filename, names):
    ...
    with xr.set_options(keep_attrs=True):
        ...
        indata = xr.open_dataset(filename,
                                 decode_cf=False, mask_and_scale=False)
        outdata = indata[names]
        for name in names:
            outdata[name] = outdata[name].where(
                outdata[name]!=outdata[name]._FillValue)
            try:
                outdata[name] = ((outdata[name]-outdata[name].add_offset) *
                                 outdata[name].scale_factor)
        ...
        for dim in outdata.dims:
            ...
            dim_names[dim] = ndim
        outdata = (outdata.rename_dims(dim_names)
...
def readin_MODIS_cdnc_best(year, doy, sds=None, version='1', sat='aqua', resolution=1):
    filename = locator.search('MODIS', 'cdnc_best', year=year, doy=doy,
                              version=version, sat={
                                  'aqua': 'A', 'terra': 'T'}[sat],
...
def readin_MODIS_L2(product, year, doy, time, sds=None,
                    col=DEFAULT_COLLECTION, varind=None):
    ...
    with Dataset(filename) as ncdf:
        for varname in sds:
            ...
            for metname in ncdf.variables[varname].ncattrs():
                metadata[varname][metname] = ncdf.variables[varname].getncattr(
                    metname)
    return metadata
...
def readin_MODIS_GEOMETA(year, doy, sat='aqua'):
    ...
    filenames = locator.search('MODIS', 'GEOMETA', year=year, mon=mon,
                               day=day, sat=sat.upper())
    if len(filenames) == 0:
        raise IOError('No files for ' + str(year) +
                      ' ' + str(doy))
    return np.genfromtxt(filenames[0], skip_header=3, delimiter=',',
                         dtype=[('GranID', 'U50'), ('StartTime', 'U20'),
                                ('Archive', 'i8'),
                                ('Orbit', 'i8'), ('DayNight', 'U1'),
                                ('EastBC', 'f8'), ('NorthBC', 'f8'),
                                ('SouthBC', 'f8'), ('WestBC', 'f8'),
                                ('GRLon1', 'f8'), ('GRLon2', 'f8'),
                                ('GRLon3', 'f8'), ('GRLon4', 'f8'),
                                ('GRLat1', 'f8'), ('GRLat2', 'f8'),
                                ('GRLat3', 'f8'), ('GRLat4', 'f8')])
...
def list_available_times(product, year, doy, col=DEFAULT_COLLECTION):
    ...
    filenames = sorted(locator.search(
        'MODIS', product, year=year, doy=doy, collection=col, time='*'))
    return list(map(lambda x: x.split('/')[-1].split('.')[2], filenames))


def intersect_times(year, doy, sat, bbox, day_night=None,):
    ...
    if day_night == None:
        times = [str(entry[0]).split('.')[2]
                 for entry in metadata
                 if _intersects((entry['NorthBC'], entry['SouthBC'],
                                 entry['EastBC'], entry['WestBC']), bbox)]
    else:
        times = [str(entry[0]).split('.')[2]
                 for entry in metadata
                 if _intersects((entry['NorthBC'], entry['SouthBC'],
...
def _intersects(entry, bbox):
    ...
    intersects the bbox'''
    return ((entry[0] > bbox[1]) & (entry[1] < bbox[0]) &
            (entry[2] > bbox[3]) & (entry[3] < bbox[2]))
...
def field_interpolate(data, factor=5, dateline_check=False):
    ...
    for i in range(1, int(factor // 2) + 1):
        output[(int(factor // 2) - i),
               :] = output[int(factor // 2), :] - prediff * i

    postdiff = output[-1 * (factor // 2 + 1), :] - \
        output[-1 * (factor // 2 + 2), :]
    for i in range(1, int(factor / 2) + 1):
    ...
    prediff = output[:, (int(factor // 2) + 1)] - output[:, (int(factor // 2))]
    for i in range(1, int(factor // 2) + 1):
        output[:, (int(factor // 2) - i)] = output[:,
                                                   int(factor // 2)] - prediff * i

    postdiff = output[:, -1 * (factor // 2 + 1)] - \
        output[:, -1 * (factor // 2 + 2)]
    for i in range(1, int(factor // 2) + 1):
        output[:, -1 * (factor // 2 + 1) + i] = output[:, -
                                                       1 * (factor // 2 + 1)] + postdiff * i
