...
def download(
    ...
    if times:
        ...
        times = [
            t
            for t in times
            if ((not check(product, year, doy, t, col=col)) or force_redownload)
        ]
    ...
    local_folder = locator.get_folder(
        "MODIS", product, year=year, doy=doy, collection=col
    )
...
def download_geometa(year, doy, sat, col=DEFAULT_COLLECTION, force_redownload=False):
    ...
    laads_file = (
        ...
        + "{col}/{sat}/{year}/M{si}D03_{year}-{mon:0>2}-{day:0>2}.txt".format(
            ...
            mon=mon,
            day=day,
            si={"AQUA": "Y", "TERRA": "O"}[sat],
        )
    )

    local_file = locator.format_filename(
        "MODIS", "GEOMETA", year=year, doy=doy, sat=sat
    )
