...
Utility functions for listing, parsing, and reading EarthCARE
files (HDF5 or ZIP).

Typical filename
----------------
ECA_EXBC_ATL_NOM_1B_20260120T183221Z_20260120T200605Z_09364D.ZIP
...
def files_available(product,
    ...
    # it is any extension .h5, .ZIP etc
    return sorted(filenames)


def variables_available(product, 
                        year, doy,
                        orbit='*****', frame='*',
                        baseline=DEFAULT_BASELINE):
    """
...
def get_orbit_filename(product, orbit,
    ...
    if not matches:
        raise FileNotFoundError(f"No file found for orbit {orbit} in geometa: {geometa_path}")
...
def get_orbit_date(orbit):
    ...
    for year in [est_year, est_year + 1]:
        try:
            path = get_or_create_earthcare_geometa(orbit=orbit, year=year)
            with open(path) as f:
                for line in f:
...
def filename_to_datetime(fname):
    """
    Extracts start datetime from an EarthCARE filename.
    """
    fname = os.path.basename(fname)
    start_str = fname.split("_")[5]
    return datetime.strptime(start_str, "%Y%m%dT%H%M%SZ")


def get_orbit_datetimes(product,
                        orbit, frame,
                        baseline=DEFAULT_BASELINE):
    """
...
def get_orbit_by_time(dtime: datetime,
    ...
    frame = best["filename"].rsplit("_", 1)[-1].replace(".ZIP", "")
    return frame
